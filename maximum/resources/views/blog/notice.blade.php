            ...
            <div class="mt-5 lg:p-20 p-0 mb-4">
                ...
                @foreach ($noticia->comentarios as $comentario)
                    <div class="flex mt-5 comment-hidden comment">
                        <div class="flex-shrink-0 mr-3">
                            <img class="mt-2 rounded-full w-8 h-8 sm:w-10 sm:h-10"
                                src="{{ '/storage/' . $comentario->ruta_imagen }}" alt="">
                        </div>
                        <div class="flex-1 border rounded-lg px-4 py-2 sm:px-6 sm:py-4 leading-relaxed">
                            <strong>{{ $comentario->nombre }} {{ $comentario->apellidos }}</strong> <span
                                class="text-xs text-gray-400">{{ $comentario->pivot->created_at }}</span>
                            <p class="text-sm">
                                {{ $comentario->pivot->contenido }}
                            </p>
                        </div>
                    </div>
                @endforeach

    ...
    <div class="relative overflow-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left  my-4 rounded">
            ...
            <tbody>
                ...
                @endforeach
                @if ($modalDelete)
                    @include('livewire.metodo-pago.delete', ['tipo' => 'Método Pago', 'id' => $deleteId])
                @endif

                @if ($modalEditMetodo)
                    @include('livewire.metodo-pago.editPago', ['metodo' => $metodoEdit])
                @endif
            </tbody>
        </table>
        @if ($modalCreateMetodo)
            @include('livewire.metodo-pago.createPago')
        @endif
